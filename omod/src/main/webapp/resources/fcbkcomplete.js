...
jQuery.facebooklist = function(elem, list, complete, ajax, height, filter, newel) {
    ...
    var defaultFilter = function(input) {
        if (filter.userfilter) {
            ...
            jQuery.each(feed.children('li:not([fckb])'), function(i, val) {
                var item = jQuery(val);
    ...
    var feedFilter = function(item, name, input) {
        if (filter.userfilter) {
            if (filter.casesensetive) {
                if (name.indexOf(input) != -1) {
                    item.html(name.replace(input, '<em>' + input + '</em>'));
                    return true;
                }
            }
            else {
    ...
    var addItemFeed = function(data, input) {
        ...
        jQuery.each(data, function(i, val) {
            if (val) {
                ...
                if (feedFilter(jQuery(li), val, input)) {
                    feed.append(li);
                    counter++;
                }
            }
    ...
    var removeExisitingElements = function() {
        if (jQuery(".maininput") && jQuery("#annoninput") && jQuery(".holder")) {
            jQuery(".maininput").remove();
            jQuery("#annoninput").remove();
