...
jQuery(function(jQuery) {
    jQuery.fn.fcbkcomplete = function(opt) {
        return this.each(function() {
            ...
            function createFCBK() {
                ...
                if (options.attachto) {
                    if (typeof(options.attachto) == "object") {
                        ...
                        jQuery(options.attachto).append(holder);
                    }
            ...
            jQuery(this).bind("addItem",
                    function(event, data) {
                        addItem(data.title, data.value, 0, 0, 0);
                    });

            //public method to remove item
            jQuery(this).bind("removeItem",
                    function(event, data) {
                        var item = jQuery('#holder').children('li[rel=' + data.value + ']');
                        if (item.length) {
                            removeItem(item);
                        }
                    });
            ...
            function addItem(title, value, preadded, locked, focusme) {
                ...
                if (!preadded) {
                    jQuery("#" + elemid + "_annoninput").remove();
                    var _item;
                    addInput(focusme);
                    if (element.children("option[value=" + value + "]").length) {
                        ...
                        var _item = jQuery(document.createElement("option"));
            ...
            function bindEvents() {
                var maininput = jQuery("#" + elemid + "_annoninput").children(".maininput");
                bindFeedEvent();
                ...
                maininput.keydown(function(event) {
                    ...
                    if ((event.keyCode == 13 || event.keyCode == 9) && !checkFocusOn()) {
                        if (options.newel) {
                            var value = xssPrevent(jQuery(this).val());
            ...
            function maxItems() {
                if (options.maxitems != 0) {
                    if (jQuery('#holder').children("li.bit-box").length < options.maxitems) {
                        return true;
                    }
                    else {
                        return false;
